...
```{python, replace_protein_ids.py}
import sys

def replace_protein_ids(protein_fasta, mapping_file, output_fasta):
    protein_transcript_map = {}
    with open(mapping_file, 'r') as map_file:
        for line in map_file:
            protein_id, transcript_id = line.strip().split(' ')
            protein_transcript_map[protein_id] = transcript_id
        ...
        with open(output_fasta, 'w') as output_file:
            for line in fasta_file:
                if line.startswith('>'):
                    ...
                    if protein_id in protein_transcript_map:
                        transcript_id = protein_transcript_map[protein_id]
                        header = header.replace(protein_id, transcript_id)
...
if __name__ == '__main__':
    if len(sys.argv) != 4:
        print('Usage: python script.py protein_fasta mapping_file output_fasta')
    else:
        protein_fasta = sys.argv[1]
        mapping_file = sys.argv[2]
        output_fasta = sys.argv[3]
        replace_protein_ids(protein_fasta, mapping_file, output_fasta)
```
